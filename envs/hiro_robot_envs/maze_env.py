...
class MazeEnv(gym.Env):
  ...
  def __init__(
      ...
      maze_id=None,
      maze_height=0.5,
      maze_size_scaling=8, include_block_in_obs=False,
      *args,
      **kwargs):
    ...
    self.include_block_in_obs = include_block_in_obs
    ...
    for i in range(len(structure)):
      for j in range(len(structure[0])):
        ...
        if structure[i][j] == 1:  # Unmovable block.
          ...
          if self.include_block_in_obs:
              _expose_body_coms = [name]
          ET.SubElement(
              moveable_body, "geom",
              name="block_%d_%d" % (i, j),
              pos="0 0 0",
    ...
    if self.include_block_in_obs:
        kwargs = {**kwargs, "expose_body_coms": _expose_body_coms}
