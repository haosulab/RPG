...
class MoveBucketEnv(MS1BaseEnv):
    ...
    def _setup_cameras(self):
        ...
        self.render_camera.set_local_pose(Pose(
            p=[0, 0, 4], q=[0.70710678, 0.        , 0.70710678, 0.        ]
        ))
    ...
    def _load_articulations(self):
        ...
        self.bucket = self._load_partnet_mobility(
            fix_root_link=False, scale=self.model_info.get("scale", 1), urdf_config=urdf_config
        )
    ...
    def _load_actors(self):
        ...
        builder.add_box_collision(half_size=box_half_size, material=obj_material, density=1000)
    ...
    def _initialize_robot(self):
        ...
        # Base orientation
        perturb_orientation = self._episode_rng.uniform(low=-0.05*np.pi, high=0.05*np.pi)
    ...
    def evaluate(self, **kwargs):
        ...
        dist_bucket_to_target = np.linalg.norm(self.root_link.get_pose().p[:2] - self.target_xy)
        ...
        if dist_ee_to_bucket < 0.1:
            ...
            if dist_bucket_height < 0.03:
                ...
                if dist_bucket_to_target <= 0.3:
                    ...
                    reward += np.clip(1 - np.exp(-reward_vel), -1, np.inf) * 2 - dist_bucket_to_target * 2
            else:
