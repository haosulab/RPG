...
class RecordEpisode(gym.Wrapper):
    ...
    def __init__(
        ...
        self.save_more = save_more
        self.info_on_video = info_on_video
        self.render_mode = render_mode
        self._render_images = []
        self._render_more_info = []
    ...
    def reset(self, **kwargs):
        ...
        if self.save_more:
            ...
            self.env.render_camera.take_picture()

            ret = self.env.unwrapped._get_camera_images(
                self.env.render_camera, rgb=False, depth=True, visual_seg=True, actor_seg=True
            )
            self._render_more_info.append(ret)
        return obs
    ...
    def flush_more(self, suffix="", verbose=False):
        print(len(self._render_more_info))
        import PIL.Image as im
        import os
        if not self.save_more or len(self._render_more_info) == 0:
            return
        video_name = "{}".format(self._episode_id)

        for idx, d in enumerate(self._render_more_info):
            if idx == 0:
                continue

            img_name = f"{video_name}_{idx:03d}_depth.npy"
            np.save(os.path.join(str(self.output_dir), img_name), d['depth'])

            img_name = f"{video_name}_{idx:03d}_seg.png"
            im.fromarray(d['actor_seg'].astype(np.uint8)[..., 0]).save(os.path.join(str(self.output_dir), img_name))
