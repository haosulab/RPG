...
def _main(args, proc_id: int = 0, num_procs=1, pbar=None):
    ...
    for ind in inds:
        ...
        for _ in range(args.max_retry + 1):
            ...
            if target_control_mode is None:
                ...
                if pbar is not None:
                    pbar.reset(total=n)
                states.append([env.cabinet.get_qpos(), env.agent.get_state()])
        ...
        for i, p in enumerate(poses):
            np.savetxt(os.path.join(env.output_dir, f'0_{i+1:03d}.txt'), p)
    ...
    print(len(states))
    import torch
    torch.save(states, os.path.join(env.output_dir, 'states'))
