...
def pose_from_txt(env, pid):
    pose = np.loadtxt(os.path.join(env.output_dir, f'0_{pid:03d}.txt'))
    r = pose[:3,:3] @ np.linalg.inv(np.array([[0,-1,0],[0,0,1],[-1,0,0]]).T)
    pose[:3,:3] = r
...
poses = []
def env_render(env):
    pose_new = pose_from_txt(env, len(poses) + 1)
...
def qpos_to_pd_joint_delta_pos(controller: PDJointPosController, qpos):
    assert type(controller) == PDJointPosControllerpose
...
def parse_args():
    ...
    parser.add_argument("--allow-failure", action="store_false")
...
def _main(args, proc_id: int = 0, num_procs=1, pbar=None):
    ...
    env = gym.make(env_id, enable_kuafu=False, **env_kwargs)
    # env = gym.make(env_id, enable_kuafu=True, **env_kwargs)
    if pbar is not None:
        pbar.set_postfix(
            {
