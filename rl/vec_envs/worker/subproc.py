    ...
    try:
        while True:
            ...
            if cmd == "step":
                ...
                env.backward(*data)
            elif cmd == "get_action_grads":
                action_grads = env.get_action_grads()
                p.send(action_grads)
...
class SubprocEnvWorker(EnvWorker):
    ...
    def zero_grad(self):
        self.parent_remote.send(['zero_grad', None])

    def backward(self, obs_grads, reward_grads):
        self.parent_remote.send(['backward', [obs_grads, reward_grads]])

    def get_action_grads(self):
        self.parent_remote.send(['get_action_grads', None])
        return self.parent_remote.recv()
