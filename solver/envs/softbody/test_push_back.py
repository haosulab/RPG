...
class CenterObstaclePush(r2d_base.Rigid2dBase):
    ...
    def sample_state_goal(self, batch_size):
        state = r2d.tensor([
            [-1.4,  0, 2, 0, 0.17,   1], # actor
            [0.0,  0, 0, 0,  0.8,  4.], # object
    ...
    def get_reward(self, s, a, s_next):
        ...
        #closer_to_goal = #(x_ball - self.goals).norm(dim=-1) 
        closer_to_goal = - (x_ball_next - self.goals).norm(dim=-1)

        actor_to_ball_next = (x_ball_next - x_actor_next).norm(dim=-1)
        r = ((- v_penalty * self.dt - actor_to_ball_next) * 100. + closer_to_goal * 100) * 0.05
        return r 
...
if __name__ == '__main__':
    ...
    # main(rpg=dict(weight=dict(reward=0.2, ent_z=200.), gd=False), env=dict(n_batches=1024), z_dim=1, path='exp/push2/pure_pg3')
    # main(rpg=dict(weight=dict(reward=0.2, ent_z=200.), gd=False), env=dict(n_batches=32), z_dim=1, path='exp/push2/pure_pg4')
    # main(rpg=dict(weight=dict(reward=0.2, ent_z=200.), gd=False), env=dict(n_batches=32), z_dim=1, path='exp/push2/pure_pg4')
    # main(rpg=dict(weight=dict(reward=0.2, ent_z=200., mutual_info=10.), gd=True), env=dict(n_batches=32), z_dim=20, path='exp/push2/rpg2')
    # main(rpg=dict(weight=dict(reward=0.2, ent_z=200., mutual_info=1.), gd=True), env=dict(n_batches=32), z_dim=20, path='exp/push2/rpg_gd2')
    main(rpg=dict(weight=dict(reward=0.2, ent_z=200., mutual_info=1.), gd=False), env=dict(n_batches=32), z_dim=20, path='exp/push2/rpg_pg')