...
def get_args(keys):
    ...
    parser.add_argument("exp", nargs='?', help=f"{keys+['all']}", default=None)
...
def run(seed_name='trainer.seed', add_path=False, default_exps=None, **kwargs):
    ...
    if args.exp is None:
        if args.filter is not None:
            ...
            args.exp = ','.join([i for i in all_exps if re.match(prog, i)])
            assert len(args.exp) > 0, "Please do not filter out all exps."
        else:
            assert default_exps is not None, f"Please provide default exps.."
            args.exp = default_exps
    ...
    for i in exps:
        ...
        if args.n_seed > 0:
            ...
            if isinstance(e, str):
                e = exp(e)
            e = e.add('--path', e.path + '/{'+seed_name.replace('.', '_')+'}').add(f'--{seed_name}', f"@'range({args.n_seed})'")
        ...
        if args.nautilus:
            ...
            if seed_name is not None and args.n_seed > 0:
                cmd += '-{' + seed_name + '}'
        ...
        cmd = exp(cmd)
        try:
            mpi_size = cmd.mpi_size
            cmd = cmd.remove('mpi_size')
            cmd = insert(str(cmd), f'--mpi_size {mpi_size}' )
        except:
            pass
